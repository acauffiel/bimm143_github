...
There are lots of way to make visualizations and plots in R. These
include so-called “base R” (like the `plot()`) and add on packages like
**ggplot2**.
...
> **N.B.** We never run an `install.packages()` in a code chunk otherwis
> we will re-install needlessly every time we render our document.

Everytime we want to use an add-on we need to load it up with a call to
`library()`
...
3.  The **geom\_** or geometry i.e. the type such as points, lines etc.
